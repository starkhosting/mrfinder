...
if(isset($_POST['submit'])){
   ...
   $update_post = $conn->prepare("UPDATE `post` SET title = ?, fees = ?, description = ?, status = ? WHERE id = ?");
   $update_post->execute([$title, $fees, $description, $status, $get_id]);
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `post` SET thumb = ? WHERE id = ?");
...
<section class="post-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      <input type="hidden" name="old_image" value="<?= $fetch_post['thumb']; ?>">
      ...
      <input type="text" name="title" maxlength="100" required placeholder="enter post title" value="<?= $fetch_post['title']; ?>" class="box">
      
      <p>service charge <span>*</span></p>
      <input type="number" name="fees" min="0" max="9999999999"  maxlength="50" required placeholder="enter service charge" value="<?= $fetch_post['fees']; ?>" class="box">
      
      <p>post description <span>*</span></p>
      <textarea name="description" class="box" required placeholder="write description" maxlength="1000" cols="30" rows="10"><?= $fetch_post['description']; ?></textarea>
      ...
      <div class="flex-btn">
         ...
         <a href="view_post.php?get_id=<?= $post_id; ?>" class="option-btn">view post</a>
      </div>
   </form>
   <?php
      } 
